...
Song-specific and anomaly-style in-play behaviour now that the generic SceneControl framework is understood. Start from the already-observed hard-coded `rivenpilgrim` widening intervention and the separate song-ID `designant` branches, and trace only enough surrounding state to identify trigger, stored state, update/revert behaviour, and any genuine gameplay/input consequences. Keep native `Tokens::DESIGNANT` Arc body mode distinct from song-ID `designant`, and keep unlocks, progression, challenge requirements and account/content-access systems out of scope unless directly required.
