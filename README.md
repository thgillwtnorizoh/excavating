...
Rendering fundamentals: establish the shared logic-note → render-note bridge for Tap, Hold, Arc, ArcTap and Flick, then follow only enough geometry/presentation state to understand visible note placement and Arc mesh construction. Use this pass to pick up deferred presentation-side details such as timinggroup `anglex`/`angley` when their actual consumers appear. Keep song-specific special effects and progression systems out of scope until the ordinary renderer foundation is clear.
