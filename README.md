...
Remaining focused gameplay refinements now that ordinary gameplay, generic SceneControls, and the first song-specific special branches are understood. Start with the exact LogicFlickNote input-rectangle construction and touch-ID release/reset lifecycle, then take Arc tracker-mode semantics, the context enabling Arc +0x170, the special camera/input mode, the exact x==+425 lane-classifier seam, and remaining TimingEvent/clock semantic identities only where they materially improve the gameplay model. Keep rendering polish separate unless a gameplay refinement directly depends on it, and keep unlocks, progression, challenge requirements and account/content-access systems out of scope.
