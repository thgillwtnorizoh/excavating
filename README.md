...
With the lane/timing semantic leftovers closed, the next focused block is rendering refinements that materially improve the gameplay model: timinggroup trace colour, exact `fadingholds` presentation behaviour, Designant secondary render state, Arc caps/particles/auxiliary nodes, and track/lane widening presentation. Keep purely cosmetic shader archaeology bounded unless it explains a gameplay-facing state. The YBN one-event green-Arc construction reason remains a small gameplay loose end worth closing when a clean native entry point appears. Unlocks, progression, challenge requirements and account/content-access systems remain out of scope unless deliberately promoted.
