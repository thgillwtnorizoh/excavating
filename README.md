...
Special in-play chart effects now that ordinary gameplay and rendering foundations are understood. Start from `LogicSceneControl` / `GameSceneVisualControlHandler` and trace the remaining scene-control families (`trackdisplay`, `redline`, `arcahvdistort`, `arcahvdebris`, `hidegroup`) only far enough to separate presentation-only effects from state that materially changes gameplay/input. Establish trigger, stored state, update/revert behavior, and bring in song-specific branches such as `rivenpilgrim` or `designant` only when directly connected. Keep unlocks, progression, challenge requirements and account systems out of scope.
