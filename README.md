...
- [`21_lane_timing_refinements.cpp`](21_lane_timing_refinements.cpp) — resolves the exact `x == +425` floor-lane dead sliver, identifies TimingEvent raw BPM versus measure beat count, reconstructs `LogicChart +0xF0/+0xF4` as selected highspeed plus BPM-normalised spatial scroll scale, preserves the `dropdead` ratingClass-2 minimum scale, and bounds the shared fallback-clock `-3000 ms` pre-roll behaviour.
- [`22_rendering_refinements.cpp`](22_rendering_refinements.cpp) — closes mechanic-facing rendering leftovers: native `tracecol` propagation and gold trace branch, exact `fadingholds` opacity feedback, common opacity virtuals, Designant root/segment opacity layers, RenderArcNote child anatomy, Arc particle nodes, Tap auxiliary opacity compensation during `enwidencamera`, and extra-lane/custom-track mask presentation.
- [`23_ybn_green_event_construction.cpp`](23_ybn_green_event_construction.cpp) — proves that YBN green Arcs do not use a special one-tick generator: colour-2 Arcs use ordinary positive-duration LogicArcNote construction, +0x170 does not affect tick/event generation, Arc +0xA0 only selects connected-continuation event indexing, zero-duration colour-2 Arcs would produce no event at all, and the observed one-unit healing behaviour is ordinary long-event quantisation followed by YBN's special +5 RR accounting.

## Next excavation area

The focused core-gameplay queue that led through YBN green-Arc construction is now closed. No archive subsystem is promoted automatically. The next section should be chosen deliberately from evidence already on the shelf, such as the separate extended chart-colour-3 conversion, Lament Rain's special-note route, CharacterAbility/gauge architecture, DORO*C, CHUNITHM gauge, or a broader SpecialScene/source-fossil pass. Archive discussions remain evidence and post-find interpretation only, not task steering. Unlocks, progression, challenge requirements and account/content-access systems remain out of scope unless deliberately promoted.
