...
With the rendering refinement slice closed, the next focused block is the remaining small gameplay loose ends. Highest-value target: prove why YBN special green Arcs generate exactly one long-event unit each and identify the construction branch that produces that event shape. Also revisit any remaining `LogicColor` or SpecialScene note-hook fields only where they materially change gameplay semantics. Archive-only systems such as Lament Rain, April Fools, DORO*C, CHUNITHM gauge and broader SpecialScene/CharacterAbility architecture remain evidence sources rather than active direction unless deliberately promoted. Unlocks, progression, challenge requirements and account/content-access systems remain out of scope unless deliberately promoted.
