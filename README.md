...
- [`19_logiccolor_arc_tracking.cpp`](19_logiccolor_arc_tracking.cpp) — identifies native `LogicColor` as the shared Arc colour/touch-ownership channel, resolves channel-3 ownership bypass and red rejection-warning state, and reconstructs the `yourbestnightmare` ratingClass-3 colour folding plus colour-2 Arc ownership exception.

## Next excavation area

Remaining focused gameplay refinements after resolving Arc colour-channel ownership. Next trace the special camera/input mode and its floor/sky branches, including the observed `-1000` and `900` thresholds and camera-derived state; then close the exact `x == +425` lane-classifier seam and remaining `LogicTimingEvent` / global-clock semantic identities only where they materially improve the gameplay model. Keep rendering polish separate unless a gameplay refinement directly depends on it, and keep unlocks, progression, challenge requirements and account/content-access systems out of scope.
